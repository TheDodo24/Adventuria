...
public class PlotCommand implements CommandExecutor, TabCompleter {
            ...
            if (args.length == 1) {
                    ...
                    if (user.checkTownMember() || p.hasPermission("towny.admin.plot.info")) {
                        ...
                        if (plot != null) {
                            ...
                            p.sendMessage("§7|----------| §6Plot " + plot.getChunk().getX() + "; " + plot.getChunk().getZ()
                                    ...
                                    "§7» " + (plot.getSetting("pvp") ? "§aPvP" : "§cPvP") + " §7|| " + (plot.getSetting("mobs") ? "§aMobs" : "§cMobs"));
                ...
                if (args[0].equalsIgnoreCase("fs")) {
                    ...
                    try {
                        ...
                        else if (!arg.equalsIgnoreCase("Infinity") && !arg.equalsIgnoreCase("-Infinity")) {
                ...
                if(args[0].equalsIgnoreCase("set")) {
                    Plot plot = Towny.getInstance().getManager().getPlotManager().get(String.valueOf(p.getLocation().getChunk().getChunkKey()));
                    if (plot != null) {
                        if (user.checkTownMember() || p.hasPermission("towny.admin.plot.set.pvp") || p.hasPermission("towny.admin.plot.set.mobs")) {
                            if((plot.isOwned() && !plot.getOwner().equals(p.getUniqueId()))
                                    ||
                                    (!plot.isOwned() &&
                                    (user.getTownRank().equals(TownRank.CITIZEN) || user.getTownRank().equals(TownRank.HELPER)))) {
                                if(!(p.hasPermission("towny.admin.plot.set.pvp") || p.hasPermission("towny.admin.plot.set.mobs"))) {
                                    if(!user.getTownRank().equals(TownRank.MAYOR) && !user.getTownRank().equals(TownRank.ASSISTANT)) {
                                        p.sendMessage(prefix + "§7Du hast nicht die Berechtigung dazu!");
                                        return true;
                                    }
                                }
                            }
                            if(!user.getTown().getKey().equalsIgnoreCase(plot.getTown().getKey())) {
                                p.sendMessage(prefix + "§7Dieses Grundstück gehört nicht zu deiner Stadt.");
                                return true;
                            }
                            switch (args[1].toLowerCase()) {
                                case "pvp":
                                    if(user.getTown().getKey().equalsIgnoreCase(plot.getTown().getKey()) || p.hasPermission("towny.admin.plot.set.pvp")) {
                                        boolean pvp;
                                        try {
                                            pvp = Boolean.parseBoolean(args[2]);
                                        } catch (Exception e) {
                                            p.sendMessage(prefix + "§6Argument 3 §7muss entweder true oder false sein.");
                                            return false;
                                        }
                                        plot.setSetting("pvp", pvp);
                                        p.sendMessage(prefix + "§6PvP §7ist auf dem Plot nun " + (pvp ? "§aerlaubt" : "§cverboten") + "§7.");
                                    } else {
                                        p.sendMessage(prefix + "§7Du hast nicht die Berechtigung dazu.");
                                    }
                                    break;
                                case "mobs":
                                    if((user.getTown().getKey().equalsIgnoreCase(plot.getTown().getKey()) && user.checkTownMember()
                                            && (user.getTownRank().equals(TownRank.ASSISTANT) || user.getTownRank().equals(TownRank.MAYOR)))
                                            || p.hasPermission("towny.admin.plot.set.mobs")) {
                                        boolean mob;
                                        try {
                                            mob = Boolean.parseBoolean(args[2]);
                                        } catch (Exception e) {
                                            p.sendMessage(prefix + "§6Argument 3 §7muss entweder true oder false sein.");
                                            return false;
                                        }
                                        plot.setSetting("mobs", mob);
                                        p.sendMessage(prefix + "§6Mobspawn §7ist auf dem Plot nun " + (mob ? "§aerlaubt" : "§cverboten") + "§7.");
                                    } else {
                                        p.sendMessage(prefix + "§7Du hast nicht die Berechtigung dazu.");
                                    }
                                    break;
                        ...
                        p.sendMessage(prefix + "§7Du stehst auf keinem Stadtgrundstück.");
                    }
                } else {
                    sendHelpMessage(s, label);
                }
    ...
    public List<String> onTabComplete(CommandSender s, Command cmd, String label, String[] args) {
        if(s instanceof Player) {
            if(args.length == 1) {
                ...
                if(args[0].equalsIgnoreCase("set"))
                    return Common.getInstance().removeAutoComplete(Lists.newArrayList("perm", "pvp", "mobs"), args[1]);
            } else if(args.length == 3) {
                if(args[0].equalsIgnoreCase("set") && args[1].equalsIgnoreCase("perm"))
                ...
                if(args[0].equalsIgnoreCase("set") && (args[1].equalsIgnoreCase("pvp") || args[1].equalsIgnoreCase("mobs")))
    ...
    private void sendSetHelpMessage(CommandSender s, String label) {
        s.sendMessage(prefix + "§e/" + label + " set §6perm [Permission] [Spielergruppe] [true/false] §7| Setzt die Permission für die Spielergruppe\n" +
                        ...
                        prefix + "§e/" + label + " set §6pvp [true/false] §7| Erlaubt/Verbietet PvP auf dem Plot\n+" +
                        prefix + "§e/" + label + " set §6mobs [true/false] §7| Erlaubt/Verbietet Mobspawn auf dem Plot");
    }
}
