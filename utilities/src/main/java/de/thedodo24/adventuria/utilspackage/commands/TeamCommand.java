...
public class TeamCommand implements CommandExecutor, TabCompleter {
                            ...
                            if(filteredTeams.size() > 0) {
                                ...
                                if (value > 0) {
                                    if(filteredTeams.size() == 1) {
                                        ...
                                        Teams t = Arrays.stream(Teams.values()).filter(team -> team.getPermName().equalsIgnoreCase(permName) ||
                                                team.getDutyName().equalsIgnoreCase(permName) || team.getLeiterGroup().equalsIgnoreCase(permName) ||
                                                (team.getLeiterGroup() + "-duty").equalsIgnoreCase(permName)).findFirst().get();
                    ...
                    if(user.isTeamHead()) {
                        ...
                        if(uTo != null) {
                            ...
                            if(user.getTeams().stream().map(Teams::getPermName).anyMatch(pName -> t.getPermName().equalsIgnoreCase(pName))) {
                                ...
                                if(Arrays.stream(Common.getInstance().getPerms().getPlayerGroups("Freebuild", op))
                                        .anyMatch(groups -> groups.equalsIgnoreCase(t.getPermName()) || groups.equalsIgnoreCase(t.getDutyName()) || groups.equalsIgnoreCase(t.getLeiterGroup()) || groups.equalsIgnoreCase(t.getLeiterGroup() + "-duty"))) {
                                    String arg = args[2];
                                    if (arg.contains(","))
                                        arg = arg.replace(",", ".");
                                    long value;
    ...
    public List<String> onTabComplete(CommandSender s, Command cmd, String label, String[] args) {
        if(args.length == 1) {
            ...
            if(s.hasPermission("team.mod"))
                r.addAll(Lists.newArrayList("sethead", "add", "kick", "log"));
